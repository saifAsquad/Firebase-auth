...
import app from "../utils/firebase"; //initalize firebase
			...
			<div className="flex flex-col items-center justify-center">
				...
				<div className="bg-white shadow rounded lg:w-1/3  md:w-1/2 w-full p-10 mt-16">
					...
					<form onSubmit={(e) => handleSubmit(e)}>
						...
						<div className="text-center w-full text-md mt-4">
							<h1 className="text-green-500">
								{success ? "Logged In Successfuly" : ""}
							</h1>
							<h1 className="text-red-500 "> {error ? error : ""}</h1>
						</div>
						<div className={`mt-8 ${success ? "hidden" : ""}`}>
							<button
								disabled={loading || success ? true : false}
								role="button"
								className={` disabled:cursor-not-allowed disabled:bg-slate-600 focus:ring-2 focus:ring-offset-2 focus:ring-indigo-700 text-sm font-semibold leading-none text-white focus:outline-none bg-indigo-700 border rounded hover:bg-indigo-600 py-4 w-full`}
							>
								Login
								<svg
									className={`${
